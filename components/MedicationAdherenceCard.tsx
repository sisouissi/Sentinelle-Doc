...
export function MedicationAdherenceCard({ patient }: MedicationAdherenceCardProps): React.ReactNode {
    ...
    const StatusIcon = ({ status }: { status: DoseStatus }) => {
        ...
        if (status === 'missed') {
         ...
         if (status === 'scheduled') {
            return <div className="w-3 h-3 rounded-full bg-slate-300" title={t('medicationAdherence.scheduled')}></div>;
        }
        return <div className="w-4 h-0.5 bg-slate-200" title={t('medicationAdherence.notApplicable')}></div>;
    };
        ...
        <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl shadow-sm h-full">
            ...
            <div className="my-4">
                <div className="flex justify-between items-baseline mb-1">
                    <span className="text-sm text-slate-600 font-medium">{t('medicationAdherence.adherenceRate')} ({t('medicationAdherence.last7days')})</span>
            ...
            <div>
                <h4 className="text-sm font-semibold text-slate-600 mb-2">{t('medicationAdherence.weeklyLog')}</h4>
                 <div className="overflow-x-auto">
                    <table className="w-full text-sm text-center border-collapse">
                        <thead>
                            <tr className="bg-slate-100/70">
                                <th className="text-left rtl:text-right p-2 font-semibold text-slate-600">{t('medicationAdherence.medication')}</th>
                                {adherenceData[0]?.days.map(({ date }) => (
                                    <th key={date.toISOString()} className="p-2 font-medium text-slate-500">
                        ...
                        <tbody>
                            {adherenceData.map(({ medication, days }) => (
                                <tr key={medication.id} className="border-b border-slate-200">
